...
static HRESULT WINAPI QueryInterface(LPCLASSFACTORY UNUSED(_data), REFIID UNUSED(riid), LPVOID *ptr) {
  WINE_TRACE("\n");
...
static HRESULT WINAPI LockServer(LPCLASSFACTORY UNUSED(_data), BOOL UNUSED(_bool)) {
  WINE_TRACE("\n");
  return S_OK;
}
...
BOOL WINAPI DllMain(HINSTANCE hinst, DWORD reason, LPVOID UNUSED(*_lp)) {
  WINE_TRACE("%d\n", reason);
