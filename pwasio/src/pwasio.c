...
static struct spa_thread *_thread_utils_create(void *_data,
                                               const struct spa_dict UNUSED(*_sd),
                                               void *(*start)(void *),
                                               void *arg) {
...
static int _thread_utils_join(void *_data, struct spa_thread UNUSED(*_st), void **retval) {
  struct thread *t = _data;
  if (!t->handle)
    return -1;
...
static int _thread_utils_get_rt_range(void UNUSED(*_data), const struct spa_dict UNUSED(*_sd), int *min,
                                      int *max) {
  *min = THREAD_PRIORITY_NORMAL;
  *max = THREAD_PRIORITY_TIME_CRITICAL;
  return 0;
}
static int _thread_utils_acquire_rt(void *_data, struct spa_thread UNUSED(*_st),
                                    int priority) {
  struct thread *t = _data;
  int err = 0;
...
static int _thread_utils_drop_rt(void *_data, struct spa_thread UNUSED(*_st)) {
  struct thread *t = _data;
  if (t->priority) {
...
STDMETHODIMP_(LONG32) Init(struct asio *_data, void UNUSED(*_void)) {
  WINE_TRACE("\n");
  struct pwasio *pwasio = (struct pwasio *)_data;
...
STDMETHODIMP_(VOID) GetDriverName(struct asio UNUSED(*_asio), PSTR name) {
  WINE_TRACE("%p\n", name);
  strcpy(name, "pwasio");
}

STDMETHODIMP_(LONG32) GetDriverVersion(struct asio UNUSED(*_asio)) {
  WINE_TRACE("\n");
...
HRESULT WINAPI CreateInstance(LPCLASSFACTORY _data, LPUNKNOWN outer, REFIID UNUSED(_riid),
                              LPVOID *ptr) {
  WINE_TRACE("\n");
  if (outer)
