...
#pwasio_dll_MODULE = pwasio_$(M).dll
...
CEXTRA = -m$(M) -D_REENTRANT -fPIC -Wall -pipe -std=gnu2x
...
pwasio_res_OBJS     = $(pwasio_res_RC_SRCS:%.rc=build-$(M)/%.res)
...
build-$(M)/$(pwasio_dll_MODULE): $(pwasio_dll_OBJS) $(pwasio_res_OBJS)
	$(WINEBUILD) -m$(M) --dll --fake-module -E pwasio.dll.spec $^ -o $@
	$(WINEBUILD) --builtin $@ 2> /dev/null

build-$(M)/$(pwasio_dll_MODULE:%.dll=%.so): $(pwasio_dll_OBJS) $(pwasio_res_OBJS)
	$(WINECC) -shared -m$(M) --winebuild=$(WINEBUILD) $^ $(pwasio_dll_LDFLAGS) \
